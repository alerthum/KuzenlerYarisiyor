...
import {
  SOCIAL_CITIZENSHIP_FAMILIES,
  SC_MISREAD
} from '../js/content/families/social-citizenship-families.js';
import {
  FAMILY_TARGET,
...
test('social-citizenship aileleri yapısal olarak geçerlidir', () => {
  for (const family of SOCIAL_CITIZENSHIP_FAMILIES) {
    const result = validateFamilyDefinition(family);
    assert.ok(result.ok, `${family.familyId}: ${result.errors.join(', ')}`);
  }
});

test('social-citizenship Aşama 04 hedeflerine ulaşır: 12/4/3', () => {
  const report = capacityReport(SOCIAL_CITIZENSHIP_FAMILIES);
...
test('familyId/skeletonId benzersizdir ve sc-* önekine sahiptir', () => {
  const familyIds = SOCIAL_CITIZENSHIP_FAMILIES.map((f) => f.familyId);
  assert.equal(new Set(familyIds).size, familyIds.length);
  assert.equal(familyIds.length, 12);
  for (const id of familyIds) assert.match(id, /^sc-/);
  const skeletonIds = SOCIAL_CITIZENSHIP_FAMILIES.flatMap((f) => f.skeletons.map((s) => s.skeletonId));
    ...
    for (const skeleton of family.skeletons) {
      for (const pathId of skeleton.reasoningPathIds) {
        for (let trial = 0; trial < 3; trial += 1) {
          ...
          assert.ok(round.questionKey?.startsWith('social-citizenship:'), `${skeleton.skeletonId}/${pathId}: questionKey`);
...
test('rights-duties forced ayrim zorunluluğu', () => {
  const family = SOCIAL_CITIZENSHIP_FAMILIES.find((f) => f.familyId === 'sc-rights-duties');
  ...
  for (let trial = 0; trial < 10; trial += 1) {
    ...
    assert.equal(round.options[round.answerIndex], 'ayrim');
  }
});

test('misread-taxonomy dört yanlış-okuma türünü kapsar', () => {
  const fam = SOCIAL_CITIZENSHIP_FAMILIES.find((f) => f.familyId === 'sc-misread-taxonomy');
  ...
  for (const v of Object.values(SC_MISREAD)) {
    assert.ok(seen.has(v), `eksik: ${v}`);
  }
});

test('generateFromFamilies sessionLength:10 üretir', () => {
  const { rounds, audit } = generateFromFamilies(SOCIAL_CITIZENSHIP_FAMILIES, { seed: 424242, count: 10 });
  assert.equal(rounds.length, 10);
  assert.ok(audit.produced >= 10);
  for (const round of rounds) {
    assert.ok(round.familyId?.startsWith('sc-'));
    assert.equal(round.kind, 'choice');
  }
});

test('CANLI: createGameSession social-citizenship yeni motor (sessionLength 10)', () => {
  const profile = { id: 'sc-live-1', age: 11, grade: 5, skills: {} };
  const session = createGameSession('social-citizenship', profile, 808080, { completedSessionCount: 1 });
  assert.equal(session.rounds.length, 10);
  const familyRounds = session.rounds.filter((r) => r.familyId?.startsWith('sc-') && r.skeletonId);
  assert.ok(familyRounds.length >= 8, `sc-* çoğunluk bekleniyor, alınan: ${session.rounds.map((r) => r.familyId).join(',')}`);
...
test('öğrenci geçmişi aynı iskeleti erken tekrar seçmez', () => {
  const profile = { id: 'sc-cool-1', age: 11, grade: 5, skills: {} };
  const first = createGameSession('social-citizenship', profile, 111, { completedSessionCount: 1 });
  const used = new Set(first.rounds.map((r) => r.skeletonId).filter(Boolean));
  const attempts = first.rounds.map((r) => ({ gameId: 'social-citizenship', skeletonId: r.skeletonId, familyId: r.familyId }));
  const second = createGameSession('social-citizenship', profile, 222, {
...
test('equality spot eşitlik ihlalini yakalar', () => {
  const fam = SOCIAL_CITIZENSHIP_FAMILIES.find((f) => f.familyId === 'sc-equality');
  const spot = fam.skeletons.find((s) => s.skeletonId.endsWith(':spot-violation'));
  const traps = new Set(['ayrimci', 'esit']);
  for (let i = 0; i < 12; i += 1) {
    const round = spot.generate(seededRandomLike(i * 7 + 2), 'raw-letters');
    const ans = round.options[round.answerIndex];
    assert.ok(traps.has(ans), `spot cevabı eşitlik ihlali etiketi olmalı: ${ans}`);
  }
});
