...
const priorityByCourse=Object.freeze({'matematik':1,'fen-bilimleri':2,'t-c-inkilap-tarihi-ve-ataturkculuk':3,'din-kulturu-ve-ahlak-bilgisi':4,'ingilizce':5,'yabanci-dil':5,'bilisim-teknolojileri-ve-yazilim':6,'sosyal-bilgiler':7,'turkce':8,'gorsel-sanatlar':9,'muzik':9});
...
export function auditAssessmentV2HumanReviewQueue(queue=buildAssessmentV2HumanReviewQueue()){
  const errors=[];
  if(queue.metrics.total!==2327)errors.push(`total:${queue.metrics.total}`);
  if(queue.metrics.approved!==5)errors.push(`approved:${queue.metrics.approved}`);
  if(queue.metrics.pending!==2322)errors.push(`pending:${queue.metrics.pending}`);
