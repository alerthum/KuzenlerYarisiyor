...
test('ürün portföyü 1-12 hedefini ve mevcut 42 ders motorunu dürüst sayaçlarla gösterir', () => {
  ...
  assert.equal(portfolio.summary.activeEngineCellCount, 42);
  assert.equal(portfolio.summary.canonicalQuestionCount, 2134);
  assert.equal(portfolio.summary.curriculumOutcomeRecordCount, 2109);
  assert.equal(portfolio.summary.coveredOutcomeCount, 2109);
});
...
test('insan incelemesi, oyun uyarlaması ve ürün yayını ayrı kapılar olarak kalır', () => {
  ...
  assert.equal(portfolio.summary.humanReviewQueueCount, 2129);
...
test('ders motorları ortak üreticiye dönüşmeden alan kimliklerini korur', () => {
  ...
  assert.equal(new Set(portfolio.engines.map(row => row.misconceptionCatalog)).size, 42);
