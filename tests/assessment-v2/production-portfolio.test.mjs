...
test('ürün portföyü 1-12 hedefini ve mevcut 67 ders motorunu dürüst sayaçlarla gösterir', () => {
  ...
  assert.equal(portfolio.summary.activeEngineCellCount, 67);
  assert.equal(portfolio.summary.canonicalQuestionCount, 2667);
  assert.equal(portfolio.summary.curriculumOutcomeRecordCount, 2642);
  assert.equal(portfolio.summary.coveredOutcomeCount, 2642);
});
...
test('insan incelemesi, oyun uyarlaması ve ürün yayını ayrı kapılar olarak kalır', () => {
  ...
  assert.equal(portfolio.summary.humanReviewQueueCount, 2662);
...
test('ders motorları ortak üreticiye dönüşmeden alan kimliklerini korur', () => {
  ...
  assert.equal(new Set(portfolio.engines.map(row => row.misconceptionCatalog)).size, 67);
