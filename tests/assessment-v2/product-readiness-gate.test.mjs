...
test('aktif otuz motorun kendi kapsamı tam olsa da 112 hücre hedefi açıktır',()=>{const result=evaluateAssessmentV2ProductReadiness({portfolio:ASSESSMENT_V2_PRODUCTION_PORTFOLIO});assert.equal(result.checks.find(c=>c.id==='active-engine-curriculum').passed,true);assert.equal(result.checks.find(c=>c.id==='course-cell-coverage').passed,false);assert.equal(result.productReady,false);});
