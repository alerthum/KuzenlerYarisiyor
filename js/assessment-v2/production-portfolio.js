...
import { MIDDLE_SCHOOL_TYMM_PHYSICAL_EDUCATION_ENGINE_RECORDS } from './middle-school-tymm-physical-education-engines.js';
...
export function buildAssessmentV2ProductionPortfolio(){
  ...
  const middlePhysicalEducationRows=MIDDLE_SCHOOL_TYMM_PHYSICAL_EDUCATION_ENGINE_RECORDS.map(record=>engineRow({id:`tr-g${record.grade}-beden-egitimi-ve-spor-tymm-v2`,grade:record.grade,courseId:'beden-egitimi-ve-spor',courseName:'Beden Eğitimi ve Spor',programFamily:'TYMM',officialOutcomeCount:record.officialOutcomeCount,coveredOutcomeCount:record.audit.metrics.implementedOutcomeCount,items:record.items,engineType:'safe physical performance, fitness inquiry, tactics and sports culture engine',verifierType:'independent safety contract, teacher observation and analytic rubric verification',misconceptionCatalog:`g${record.grade}-physical-education-misconceptions-v1`,nextAction:`${record.items.length} görevi beden eğitimi öğretmeni, güvenli alan, ekipman, kapsayıcılık ve yaş uygunluğu açısından kalibre et.`,blockers:[`${record.items.length} görev insan incelemesi bekliyor.`,'Gerçek güvenli alan, yaşa uygun ekipman ve öğretmen gözlem rubrikleri tamamlanmadı.']}));
  const COURSE_META_G4=Object.freeze({turkce:{engineType:'grade4 language evidence and production engine',verifierType:'independent language evidence and rubric verification',misconceptionCatalog:'g4-turkish-misconceptions-v1'},matematik:{engineType:'grade4 mathematical reasoning and modelling engine',verifierType:'independent representation and rubric verification',misconceptionCatalog:'g4-math-misconceptions-v1'},'sosyal-bilgiler':{engineType:'grade4 source, map and civic inquiry engine',verifierType:'independent source and rubric verification',misconceptionCatalog:'g4-social-misconceptions-v1'},'insan-haklari-vatandaslik-ve-demokrasi':{engineType:'grade4 rights, democracy and peaceful dialogue engine',verifierType:'independent civic evidence and rubric verification',misconceptionCatalog:'g4-human-rights-misconceptions-v1'},'gorsel-sanatlar':{engineType:'grade4 visual production and critique engine',verifierType:'independent visual rubric and provenance verification',misconceptionCatalog:'g4-visual-arts-misconceptions-v1'},muzik:{engineType:'grade4 listening, performing and creating engine',verifierType:'independent audio and music rubric verification',misconceptionCatalog:'g4-music-misconceptions-v1'},'beden-egitimi-ve-oyun':{engineType:'grade4 safe movement and game performance engine',verifierType:'independent safety and performance rubric verification',misconceptionCatalog:'g4-physical-play-misconceptions-v1'},'trafik-guvenligi':{engineType:'grade4 traffic scenario and first-aid safety engine',verifierType:'independent safety scenario and rubric verification',misconceptionCatalog:'g4-traffic-misconceptions-v1'}});
  const grade4PreTymmRows=GRADE4_PRE_TYMM_ENGINE_RECORDS.map(record=>engineRow({id:`tr-g4-${record.courseId}-pre-tymm-v2`,grade:4,courseId:record.courseId,courseName:record.courseName,programFamily:'PRE_TYMM',officialOutcomeCount:record.outcomes.length,coveredOutcomeCount:record.audit.metrics.implementedOutcomeCount,items:record.items,engineType:COURSE_META_G4[record.courseId].engineType,verifierType:COURSE_META_G4[record.courseId].verifierType,misconceptionCatalog:COURSE_META_G4[record.courseId].misconceptionCatalog,nextAction:`${record.items.length} görevi sınıf/alan öğretmeni, yaş uygunluğu, gerçek materyal ve güvenlik açısından kalibre et.`,blockers:[`${record.items.length} görev insan incelemesi bekliyor.`,'Gerçek ses, görsel, hareket alanı, sanat ürünü, harita veya güvenlik simülasyonu gereken görevlerin varlıkları tamamlanmadı.']}));
...
export function auditAssessmentV2ProductionPortfolio(portfolio=buildAssessmentV2ProductionPortfolio()){
  ...
  if(portfolio.engines.length!==67)errors.push(`engine-count:${portfolio.engines.length}`);
  if(new Set(portfolio.engines.map(r=>r.id)).size!==portfolio.engines.length)errors.push('duplicate-engine-id');
  if(portfolio.summary.activeEngineCellCount!==67)errors.push(`active-cell-count:${portfolio.summary.activeEngineCellCount}`);
  ...
  if(portfolio.summary.canonicalQuestionCount!==2667)errors.push(`question-count:${portfolio.summary.canonicalQuestionCount}`);
  if(portfolio.summary.curriculumOutcomeRecordCount!==2642)errors.push(`outcome-record-count:${portfolio.summary.curriculumOutcomeRecordCount}`);
  if(portfolio.summary.coveredOutcomeCount!==2642)errors.push(`covered-outcome-count:${portfolio.summary.coveredOutcomeCount}`);
  ...
  if(portfolio.summary.humanReviewQueueCount!==2662)errors.push(`review-queue:${portfolio.summary.humanReviewQueueCount}`);
