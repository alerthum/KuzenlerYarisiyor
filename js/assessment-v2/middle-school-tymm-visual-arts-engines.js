import { GRADE5_VISUAL_ARTS_OUTCOMES_TYMM_2024 } from '../curriculum/outcomes/tr-g5-gorsel-sanatlar-tymm-2024.js';
import { GRADE6_VISUAL_ARTS_OUTCOMES_TYMM_2024 } from '../curriculum/outcomes/tr-g6-gorsel-sanatlar-tymm-2024.js';
import { GRADE7_VISUAL_ARTS_OUTCOMES_TYMM_2024 } from '../curriculum/outcomes/tr-g7-gorsel-sanatlar-tymm-2024.js';
...
function visualArtsSpec(outcome,index){
  ...
  return {
    ...
    batch:`PHASE4X_G${outcome.grade}_PRIMARY_VISUAL_ARTS`
  };
}

const DEFINITIONS=freeze([
  [5,GRADE5_VISUAL_ARTS_OUTCOMES_TYMM_2024],
  [6,GRADE6_VISUAL_ARTS_OUTCOMES_TYMM_2024],
  [7,GRADE7_VISUAL_ARTS_OUTCOMES_TYMM_2024]
]);
...
export function buildMiddleSchoolTymmVisualArtsTasks(){return freeze(BUILT.flatMap(record=>record.items));}
export const MIDDLE_SCHOOL_TYMM_VISUAL_ARTS_ENGINE_RECORDS=freeze(BUILT.map(record=>freeze({grade:record.grade,courseId:record.courseId,officialOutcomeCount:record.outcomes.length,items:record.items,engine:record.engine,audit:record.audit})));
export const MIDDLE_SCHOOL_TYMM_VISUAL_ARTS_AUDIT=freeze({ok:BUILT.every(record=>record.audit.ok),metrics:freeze({engineCount:3,officialOutcomeCount:30,itemCount:30,humanReviewStatus:'NOT_MEASURED',realArtworkStatus:'REQUIRED_NOT_ATTACHED',gameAdaptationAllowed:false})});
