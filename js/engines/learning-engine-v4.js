...
const THINKING_PATTERN_BY_FAMILY = {
  'multi-clue-order':'ORDERING_TABLE', 'weekly-schedule':'TIMELINE', 'conditional-team':'CONDITIONAL_SELECTION',
  'three-truth-liars':'TRUTH_CONSISTENCY', 'two-step-code':'ALGORITHMIC_TRANSFORM', 'direction-route':'SPATIAL_TRACKING',
  'venn-classification':'SET_CLASSIFICATION', 'book-owner-matching':'MATCHING_TABLE', 'binary-switches':'STATE_TRANSITION',
  'constraint-table':'CONSTRAINT_SCHEDULING', 'three-digit-code':'MULTI_CONSTRAINT_ELIMINATION',
  'set-logic-no-overlap':'SET_INFERENCE', 'linear-seating-elimination':'SEATING_ELIMINATION',
  'shape-color-matrix':'ATTRIBUTE_MATRIX', 'meeting-day-intersection':'TIMELINE_INTERSECTION',
  'nested-containers':'TRANSITIVE_RELATION', 'conditional-contrapositive':'CONDITIONAL_CHAIN'
};

const V10_WEAK_LOGIC_FAMILIES_FOR_UPPER_GRADES = new Set([
  'binary-switches','three-digit-code','set-logic-no-overlap','conditional-contrapositive','nested-containers','meeting-day-intersection'
]);

function thinkingPatternFor(familyId, tags = []) {
  return THINKING_PATTERN_BY_FAMILY[familyId] || String(tags[0] || familyId || 'GENERAL_REASONING').toUpperCase().replace(/[^A-Z0-9ÇĞİÖŞÜ]+/g,'_');
}
...
function optionQuestion({ familyId, minGrade, maxGrade, cognitiveDepth, role, prompt, context = '', options, answerValue, explanation, timeLimit = null, visual = null, hints = [], tags = [], teachingSolution = null }) {
  ...
  return {
    ...
    thinkingPatternId: thinkingPatternFor(familyId, tags), interactionModel: visual ? 'VISUAL' : 'TEXTUAL',
...
function validateChallengeQuestion(question) {
  ...
  const normalizedContext=String(question.context||'').toLocaleLowerCase('tr-TR').replace(/[^a-zçğıöşü0-9 ]/g,' ');
  const normalizedAnswer=String(question.answerValue||'').toLocaleLowerCase('tr-TR').replace(/[^a-zçğıöşü0-9 ]/g,' ').trim();
  if(normalizedAnswer.length>8 && normalizedContext.includes(normalizedAnswer))errors.push('cevap soru metninde doğrudan sızıyor');
  if(!question.thinkingPatternId)errors.push('thinkingPatternId eksik');
...
function buildDiverseSession({ gameId, catalog, profile, seed, count, seenQuestionKeys, recentFamilyIds = [] }) {
  ...
  const thinkingPatterns = new Set();
  ...
  for (let slot = 0; slot < rolePlan.length && rounds.length < count; slot += 1) {
    ...
    const upperGradeAllowed = (factory) => !(gameId === 'logic-station' && baseGrade >= 8 && V10_WEAK_LOGIC_FAMILIES_FOR_UPPER_GRADES.has(factory.id));
    const exact = catalog.filter((factory) => factory.minGrade <= targetGrade && factory.maxGrade >= targetGrade && isChallengeFamilyAllowed(factory, gameId) && upperGradeAllowed(factory));
    const nearby = catalog.filter((factory) => factory.minGrade <= Math.min(12, targetGrade + 1) && factory.maxGrade >= Math.max(1, targetGrade - 1) && isChallengeFamilyAllowed(factory, gameId) && upperGradeAllowed(factory));
    ...
    for (let candidateIndex = 0; candidateIndex < candidates.length && !selected; candidateIndex += 1) {
      ...
      for (let retry = 0; retry < 96 && !selected; retry += 1) {
        ...
        if (thinkingPatterns.has(question.thinkingPatternId)) continue;
    ...
    if (selected) {
      ...
      thinkingPatterns.add(selected.thinkingPatternId);
    }
  }
  ...
  if (rounds.length < count) {
    const fallbackCandidates = shuffle(catalog.filter((factory) => !families.has(factory.id) && !(gameId === 'logic-station' && baseGrade >= 8 && V10_WEAK_LOGIC_FAMILIES_FOR_UPPER_GRADES.has(factory.id)) && factory.minGrade <= baseGrade + 1 && factory.maxGrade >= Math.max(1, baseGrade - 1) && isChallengeFamilyAllowed(factory, gameId)), random)
      .sort((a, b) => Number(recent.has(a.id)) - Number(recent.has(b.id)) || b.depth - a.depth);
    for (const factory of fallbackCandidates) {
      ...
      const key = questionSignature(gameId, question);
      if (seen.has(key) || thinkingPatterns.has(question.thinkingPatternId) || validateChallengeQuestion(question).length) continue;
      question.questionKey = key;
      question.targetGrade = baseGrade;
      question.curriculumRole = 'current';
      rounds.push(question);
      families.add(factory.id);
      thinkingPatterns.add(question.thinkingPatternId);
    }
  }
  return rounds;
}
...
export function auditCognitiveSession(rounds = []) {
  const errors=[]; const families=new Set(); const patterns=new Set();
  for(const [index,q] of rounds.entries()){
    for(const error of validateChallengeQuestion(q)) errors.push(`${index+1}. soru: ${error}`);
    if(families.has(q.familyId)) errors.push(`${index+1}. soru: aynı soru ailesi tekrar ediyor`);
    if(patterns.has(q.thinkingPatternId)) errors.push(`${index+1}. soru: aynı düşünme kalıbı tekrar ediyor`);
    families.add(q.familyId); patterns.add(q.thinkingPatternId);
  }
  return { ok: errors.length===0, errors, familyCount:families.size, thinkingPatternCount:patterns.size };
}
