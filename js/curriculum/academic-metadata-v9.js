...
const TOPIC_INFERENCE_RULES = Object.freeze({
  'problem-hunter': [
    ['functions', /fonksiyon|f\s*\(|grafi(?:k|ği).*değer/i],
    ['percent', /yüzde|%|indirim|zam|kâr|kar-zarar/i],
    ['ratio', /oran|orantı|ölçek/i],
    ['equations', /denklem|bilinmeyen|x/i],
    ['probability', /olasılık|ihtimal/i],
    ['data', /grafik|tablo|veri/i],
    ['word-problems', /yaş|hareket|hız|işçi|havuz|karışım/i]
  ],
  'geometry-lab': [
    ['measurement', /alan|çevre|hacim|uzunluk/i],
    ['geometry', /açı|üçgen|dörtgen|çember|daire/i],
    ['spatial-reasoning', /katla|döndür|izdüşüm|uzamsal/i]
  ],
  'paragraph-detective': [
    ['main-idea', /ana düşünce|ana fikir/i],
    ['inference', /çıkarılabilir|ulaşılabilir|yargı/i],
    ['evidence', /kanıt|gerekçe|destek/i],
    ['reading-comprehension', /parça|metin/i]
  ],
  'english-cloze': [
    ['grammar', /tense|grammar|fiil|zaman/i],
    ['context', /dialogue|diyalog|context|bağlam/i],
    ['vocabulary', /word|kelime|meaning|anlam/i]
  ],
  'logic-station': [
    ['ordering', /sıra|önce|sonra|yan yana/i],
    ['conditions', /koşul|şart|yalnızca|en az|en çok/i],
    ['deduction', /kesinlikle|çıkarım|olamaz/i],
    ['logic', /mantık|eşleştirme/i]
  ],
  'olympiad-ladder': [
    ['number-theory', /kalan|bölünebil|asal|tam sayı/i],
    ['combinatorics', /kaç farklı|seçim|diziliş|sayma/i],
    ['geometry', /üçgen|çember|açı|alan/i],
    ['inequality', /eşitsiz|en büyük|en küçük/i],
    ['proof', /kanıtla|ispat/i]
  ]
});

function inferTopicId(gameId, payload, definition, questionKey) {
  if (payload.topicId) return payload.topicId;
  const text = `${payload.prompt || ''} ${payload.context || ''} ${payload.explanation || ''} ${payload.familyId || ''}`;
  const rules = TOPIC_INFERENCE_RULES[gameId] || [];
  for (const [topicId, pattern] of rules) if (pattern.test(text)) return topicId;
  return stablePick(definition.topicIds, questionKey);
}
...
export function normalizeAcademicAttempt(payload = {}) {
  ...
  const topicId = inferTopicId(payload.gameId, payload, definition, questionKey);
