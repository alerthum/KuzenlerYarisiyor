...
test('ürün portföyü 1-12 hedefini ve mevcut 24 ders motorunu dürüst sayaçlarla gösterir', () => {
  ...
  assert.equal(portfolio.summary.activeEngineCellCount, 24);
  assert.equal(portfolio.summary.canonicalQuestionCount, 1491);
  assert.equal(portfolio.summary.curriculumOutcomeRecordCount, 1466);
  assert.equal(portfolio.summary.coveredOutcomeCount, 1466);
});
...
test('insan incelemesi, oyun uyarlaması ve ürün yayını ayrı kapılar olarak kalır', () => {
  ...
  assert.equal(portfolio.summary.humanReviewQueueCount, 1486);
...
test('ders motorları ortak üreticiye dönüşmeden alan kimliklerini korur', () => {
  assert.equal(new Set(portfolio.engines.map(row => row.engineType)).size, 18);
  assert.equal(new Set(portfolio.engines.map(row => row.misconceptionCatalog)).size, 24);
