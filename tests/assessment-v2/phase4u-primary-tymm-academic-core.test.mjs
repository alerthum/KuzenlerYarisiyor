...
test('aktif motor hücreleri ders çizelgesine bağlı ve portföy sayaçları dürüsttür',()=>{
  ...
  assert.equal(ASSESSMENT_V2_PRODUCTION_PORTFOLIO.summary.activeEngineCellCount,56);
  assert.equal(ASSESSMENT_V2_PRODUCTION_PORTFOLIO.summary.activeGradeCount,8);
  assert.equal(ASSESSMENT_V2_PRODUCTION_PORTFOLIO.summary.curriculumOutcomeRecordCount,2302);
  assert.equal(ASSESSMENT_V2_PRODUCTION_PORTFOLIO.summary.canonicalQuestionCount,2327);
  assert.equal(ASSESSMENT_V2_PRODUCTION_PORTFOLIO.summary.humanReviewQueueCount,2322);
