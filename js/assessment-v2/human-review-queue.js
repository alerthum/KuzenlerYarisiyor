...
const priorityByCourse=Object.freeze({'matematik':1,'fen-bilimleri':2,'turkce':3});
...
export function auditAssessmentV2HumanReviewQueue(queue=buildAssessmentV2HumanReviewQueue()){
  const errors=[];
  if(queue.metrics.total!==161)errors.push(`total:${queue.metrics.total}`);
  if(queue.metrics.approved!==5)errors.push(`approved:${queue.metrics.approved}`);
  if(queue.metrics.pending!==156)errors.push(`pending:${queue.metrics.pending}`);
