...
test('ürün portföyü 1-12 hedefini ve mevcut 45 ders motorunu dürüst sayaçlarla gösterir', () => {
  ...
  assert.equal(portfolio.summary.activeEngineCellCount, 45);
  assert.equal(portfolio.summary.canonicalQuestionCount, 2167);
  assert.equal(portfolio.summary.curriculumOutcomeRecordCount, 2142);
  assert.equal(portfolio.summary.coveredOutcomeCount, 2142);
});
...
test('insan incelemesi, oyun uyarlaması ve ürün yayını ayrı kapılar olarak kalır', () => {
  ...
  assert.equal(portfolio.summary.humanReviewQueueCount, 2162);
...
test('ders motorları ortak üreticiye dönüşmeden alan kimliklerini korur', () => {
  ...
  assert.equal(new Set(portfolio.engines.map(row => row.misconceptionCatalog)).size, 45);
