...
const places = ['okul kütüphanesi', 'bilim merkezi', 'mahalle parkı', 'spor salonu', 'müze atölyesi', 'robotik kulübü', 'doğa gözlem alanı', 'sanat atölyesi'];
const colors = ['mavi', 'yeşil', 'turuncu', 'mor', 'sarı', 'kırmızı'];


function applySurfaceVariant(candidate, random) {
  const person = pick(people, random);
  const place = pick(places, random);
  const document = pick(['inceleme günlüğünde', 'kulüp raporunda', 'okul gazetesi için hazırladığı notta', 'proje dosyasında', 'sunum taslağında', 'araştırma kartında'], random);
  const color = pick(colors, random);
  const frame = `${person}, ${place} çalışması için hazırladığı ${color} başlıklı ${document} şu bilgilere yer verdi:`;
  return { ...candidate, context: `${frame} ${candidate.context}` };
}
  ...
  for (let index=0; index<plan.length; index+=1) {
    ...
    for (const factory of ordered) {
      for (let attempt=0; attempt<160; attempt+=1) {
        ...
        const candidate=applySurfaceVariant(factory.create(random,role), random);
