...
test('ürün portföyü 1-12 hedefini ve mevcut 12 ders motorunu dürüst sayaçlarla gösterir', () => {
  ...
  assert.equal(portfolio.summary.activeEngineCellCount, 12);
  assert.equal(portfolio.summary.canonicalQuestionCount, 717);
  assert.equal(portfolio.summary.curriculumOutcomeRecordCount, 692);
  assert.equal(portfolio.summary.coveredOutcomeCount, 692);
});
...
test('insan incelemesi, oyun uyarlaması ve ürün yayını ayrı kapılar olarak kalır', () => {
  ...
  assert.equal(portfolio.summary.humanReviewQueueCount, 712);
...
test('ders motorları ortak üreticiye dönüşmeden alan kimliklerini korur', () => {
  assert.equal(new Set(portfolio.engines.map(row => row.engineType)).size, 12);
  assert.equal(new Set(portfolio.engines.map(row => row.misconceptionCatalog)).size, 12);
