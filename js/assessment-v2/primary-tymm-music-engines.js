import { GRADE1_MUSIC_OUTCOMES_TYMM_2024 } from '../curriculum/outcomes/tr-g1-muzik-tymm-2024.js';
import { GRADE2_MUSIC_OUTCOMES_TYMM_2024 } from '../curriculum/outcomes/tr-g2-muzik-tymm-2024.js';
import { GRADE3_MUSIC_OUTCOMES_TYMM_2024 } from '../curriculum/outcomes/tr-g3-muzik-tymm-2024.js';
...
function musicSpec(outcome,index){
  ...
  return {
    ...
    batch:`PHASE4W_G${outcome.grade}_PRIMARY_MUSIC`
  };
}

const DEFINITIONS=freeze([
  [1,GRADE1_MUSIC_OUTCOMES_TYMM_2024],
  [2,GRADE2_MUSIC_OUTCOMES_TYMM_2024],
  [3,GRADE3_MUSIC_OUTCOMES_TYMM_2024]
]);
...
export function buildPrimaryTymmMusicTasks(){return freeze(BUILT.flatMap(record=>record.items));}
export const PRIMARY_TYMM_MUSIC_ENGINE_RECORDS=freeze(BUILT.map(record=>freeze({grade:record.grade,courseId:record.courseId,officialOutcomeCount:record.outcomes.length,items:record.items,engine:record.engine,audit:record.audit})));
export const PRIMARY_TYMM_MUSIC_AUDIT=freeze({ok:BUILT.every(record=>record.audit.ok),metrics:freeze({engineCount:3,officialOutcomeCount:37,itemCount:37,humanReviewStatus:'NOT_MEASURED',licensedAudioStatus:'REQUIRED_NOT_ATTACHED',gameAdaptationAllowed:false})});
