...
const EXPECTED = Object.freeze({
  'geometry-lab': { questions: 12, families: 6, sessionLength: 6 },
  'meaning-hunt': { questions: 10, families: 5, sessionLength: 5 },
  'social-citizenship': { questions: 22, families: 11, sessionLength: 10 }
});
const EXPECTED_GAMES = Object.keys(EXPECTED);

function allRounds(grade = 9) {
  return EXPECTED_GAMES.flatMap((gameId) => generatePremiumHighschoolRoundsP(gameId, {
    seed: 20260803,
    count: 999,
    grade
  }).rounds);
...
test('9–10 premium dalga P 22 blueprint ve 44 bağımsız doğrulanmış varyant taşır', () => {
  assert.deepEqual(PREMIUM_HIGHSCHOOL_GAME_IDS_P, EXPECTED_GAMES);
  assert.equal(premiumHighschoolBlueprintReportP.blueprintCount, 22);
  assert.equal(premiumHighschoolBlueprintReportP.variantCount, 44);
  assert.equal(premiumHighschoolBlueprintReportP.verifiedInstances, 44);
  const inventory = premiumHighschoolInventoryP();
  assert.equal(Object.values(inventory).reduce((sum, row) => sum + row.questionCount, 0), 44);
...
test('dalga P içeriğinin tamamı beş yayın kapısından ve bağımsız solverdan geçer', () => {
  const rounds = allRounds(9);
  assert.equal(rounds.length, 44);
...
test('dalga P aynı blueprint varyantlarında yapısal kimliği korur, yüzey kimliğini ayırır', () => {
...
test('9. ve 10. sınıf canlı oturumları dalga P oyunlarında yalnız premium blueprint içeriğiyle dolar', () => {
...
test('dalga P 8. ve 11. sınıfa sızmaz', () => {
  for (const gameId of EXPECTED_GAMES) {
    assert.equal(generatePremiumHighschoolRoundsP(gameId, { count: 50, grade: 8 }).rounds.length, 0, `${gameId}: grade 8 leak`);
    assert.equal(generatePremiumHighschoolRoundsP(gameId, { count: 50, grade: 11 }).rounds.length, 0, `${gameId}: grade 11 leak`);
  }
});
