...
export function buildAssessmentV2AutonomousExpansionPlan(portfolio=ASSESSMENT_V2_PRODUCTION_PORTFOLIO,queue=ASSESSMENT_V2_HUMAN_REVIEW_QUEUE){
  ...
  const backlog=Object.freeze([
    Object.freeze({order:1,id:'calibrate-risk-sample',kind:'HUMAN_REVIEW',scope:'7 aktif motor için risk tabakalı örneklem',reason:'440 görevin tümünü sırayla okumadan önce her format, kazanım ve zorluk katmanından dengeli örneklem onaylanmalı.',count:70}),
    ...
    Object.freeze({order:5,id:'expand-grade-map',kind:'NEW_ENGINE',scope:'1–12 zorunlu ders hücreleri',reason:'112 hücrenin 7’si aktif; ortak sözleşmeyle yeni sınıf-ders motorları açılmalı.',count:105})
  ]);
  return Object.freeze({schemaVersion:'2.0',generatedAt:new Date().toISOString(),target:portfolio.target,status:'LGS_CORE_COMPLETE_AUTONOMOUS_REVIEW_AND_EXPANSION_PLAN',productReady:false,gameAdaptationAllowed:false,
...
export function auditAssessmentV2AutonomousExpansionPlan(plan=buildAssessmentV2AutonomousExpansionPlan()){
  const errors=[];
  if(plan.metrics.activeEngineCount!==7)errors.push('active-engine-count');
  if(plan.metrics.engineeringScopeCompleteEngineCount!==7)errors.push(`scope-complete:${plan.metrics.engineeringScopeCompleteEngineCount}`);
  if(plan.metrics.remainingOutcomeCount!==0)errors.push(`remaining-outcomes:${plan.metrics.remainingOutcomeCount}`);
  if(plan.metrics.humanReviewQueueCount!==440)errors.push(`review-queue:${plan.metrics.humanReviewQueueCount}`);
  if(plan.metrics.unopenedCourseCellCount!==105)errors.push(`unopened-cells:${plan.metrics.unopenedCourseCellCount}`);
