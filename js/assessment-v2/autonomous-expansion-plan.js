...
export function auditAssessmentV2AutonomousExpansionPlan(plan=buildAssessmentV2AutonomousExpansionPlan()){
  const errors=[];
  if(plan.metrics.activeEngineCount!==42)errors.push('active-engine-count');
  if(plan.metrics.engineeringScopeCompleteEngineCount!==42)errors.push(`scope-complete:${plan.metrics.engineeringScopeCompleteEngineCount}`);
  if(plan.metrics.remainingOutcomeCount!==0)errors.push(`remaining-outcomes:${plan.metrics.remainingOutcomeCount}`);
  if(plan.metrics.humanReviewQueueCount!==2129)errors.push(`review-queue:${plan.metrics.humanReviewQueueCount}`);
  if(plan.metrics.unopenedCourseCellCount!==70)errors.push(`unopened-cells:${plan.metrics.unopenedCourseCellCount}`);
