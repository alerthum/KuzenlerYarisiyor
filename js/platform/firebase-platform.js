...
function adminAnalytics(schools,classrooms,learners,metrics) {
  ...
  return `<div class="module-heading"><div><span class="badge cyan">Karşılaştırmalı analiz</span><h2>Okulların ve sınıfların öğrenme nabzı</h2><p>Toplam soru, doğruluk ve ipucu yoğunluğunu aynı ölçekte karşılaştırın.</p></div></div><div class="analytics-section"><h3>Okul karşılaştırması</h3><div class="analytics-compare-list">${compareCards(schoolRows,'Okul')||'<div class="empty-state">Karşılaştırma için okul verisi yok.</div>'}</div></div><div class="analytics-section"><h3>Sınıf karşılaştırması</h3><div class="analytics-compare-list">${compareCards(classRows,'Sınıf')||'<div class="empty-state">Karşılaştırma için sınıf verisi yok.</div>'}</div></div>`;
}
...
function adminManagement(accounts,classrooms,learners,schools,reports,metrics=new Map()) {
  const renderers={
...
async function handleAction(target) {
  ...
  try {
    ...
    if (action==='admin-section') { adminSection=target.dataset.section||'overview'; adminMenuOpen=false; sessionStorage.setItem('kuzenler-admin-section',adminSection); await renderAdultPortal(); }
