...
import { buildTopicMastery } from './engines/mastery-engine-v9.js';
import { composeAdaptiveSession } from './engines/session-composer-v9.js';
import { academicDefinitionForGame } from './curriculum/academic-metadata-v9.js';
import { buildAdaptiveLearningPlan } from './engines/micro-teaching-v9.js';
...
import { diagnoseV11ChoiceResponse } from './engines/v11-misconception-profile.js';
import {
  escapeHtml,
  formatDuration,
  formatNumber,
  ...
  return `
    <main class="app-shell game-mode">
      ...
      ${ui.paused ? `<section class="paused-card"><div class="pause-icon">Ⅱ</div><h2>Soru durduruldu</h2><p>Soru gizlendi ve süre durdu. Hazır olduğunda devam et.</p><button class="primary-button" data-action="toggle-pause">Devam Et</button></section>` : `<section class="question-card">`}
        <div class="question-meta"><span class="question-label">${escapeHtml(SKILLS[round.skill])}</span><span class="question-difficulty">${session.rewardEligible ? `Zorluk ${round.difficulty}/5 • İpucusuz doğru: +${noHintXp} XP` : `XP dışı öğrenme alanı • Zorluk ${round.difficulty}/5`}</span></div>${round.adaptivePlacement ? `<div class="adaptive-route-note"><span>🧭 Kişisel rota</span><strong>${escapeHtml(topicLabel(round.topicId))}</strong><small>Bu soru gelişim dengesine sessizce eklendi.</small></div>` : ''}${round.targetGrade ? `<div class="curriculum-meta"><span>${escapeHtml(V4_QUALITY_POLICY.labels[round.curriculumRole] || 'Sınıf çalışması')}</span><span>${round.targetGrade}. sınıf hedefi</span>${round.familyId ? `<span>Aile: ${escapeHtml(round.familyId)}</span>` : ''}</div>` : ''}${round.sourceLabel ? `<div class="source-label">${escapeHtml(round.sourceLabel)}</div>` : ""}
...
function renderAdaptivePlan(attempts) {
  const plan = buildAdaptiveLearningPlan(attempts);
  if (!plan.length) return '';
  return `<section class="section panel mastery-focus-panel">
    <div class="section-header"><div><span class="badge cyan">Sessiz gelişim planı</span><h2>Bugünkü rotaya dengeli biçimde eklenen konular</h2><p>Zayıf konu bütün oturumu kaplamaz; farklı kartların içinde küçük dozlarda çalışılır.</p></div></div>
    <div class="mastery-focus-grid">${plan.map(item=>`<article class="mastery-focus-card"><div><strong>${escapeHtml(item.label)}</strong><span>${escapeHtml(item.mastery?.status || 'Keşfediliyor')}</span></div><p>${escapeHtml(item.microLesson.strategy)}</p><small>Rota payı %${Math.round(item.recommendedShare*100)} • ${item.microLesson.durationMinutes} dk mikro çalışma</small></article>`).join('')}</div>
  </section>`;
}

function renderTopicMastery(attempts) {
  const rows = buildTopicMastery(attempts);
  if (!rows.length) return '<p class="muted">İlk sorulardan sonra konu bazlı gelişim burada görünür.</p>';
  return `<div class="topic-mastery-list">${rows.slice(0,12).map(row=>`<article class="topic-mastery-row"><div class="topic-mastery-head"><strong>${escapeHtml(topicLabel(row.topicId))}</strong><span class="mastery-status">${escapeHtml(row.status)}</span></div><div class="progress-track"><div class="progress-fill" style="width:${Math.max(6,row.masteryScore)}%"></div></div><div class="topic-mastery-meta"><span>%${row.accuracy} doğruluk</span><span>${row.attempts} soru</span><span>${row.distinctDays} gün</span><span>${row.familyDiversity} soru ailesi</span></div></article>`).join('')}</div>`;
}

function renderMicroTeaching(attempts) {
  const plan = buildAdaptiveLearningPlan(attempts);
  if (!plan.length) return '<p class="muted">Şu anda özel güçlendirme gerektiren bir konu görünmüyor.</p>';
  return `<div class="micro-teaching-grid">${plan.map(item=>`<article class="micro-teaching-card"><span class="badge orange">${escapeHtml(item.mastery?.status || 'Gelişiyor')}</span><h3>${escapeHtml(item.microLesson.title)}</h3><p><strong>Strateji:</strong> ${escapeHtml(item.microLesson.strategy)}</p><p><strong>Örnek yaklaşım:</strong> ${escapeHtml(item.microLesson.example)}</p><p><strong>Dikkat:</strong> ${escapeHtml(item.microLesson.caution)}</p><small>${item.microLesson.practiceCount} uygulama • yaklaşık ${item.microLesson.durationMinutes} dakika</small></article>`).join('')}</div>`;
}
...
function renderProgress() {
  ...
  return `
    <main class="app-shell">
      ...
      <section class="section panel">
        <div class="section-header"><div><h2>Konu ustalık haritası</h2><p>Doğruluk, zorluk, ipucu, süre, gün ve soru ailesi çeşitliliği birlikte değerlendirilir.</p></div></div>
        ${renderTopicMastery(attempts)}
      </section>
      <section class="section panel">
        <div class="section-header"><div><h2>Kısa güçlendirme önerileri</h2><p>Uzun ders anlatımı yerine kısa strateji ve küçük dozlu uygulama.</p></div></div>
        ${renderMicroTeaching(attempts)}
      </section>
      <section class="section panel">
...
function startGame(gameId) {
  const profile = activeProfile();
  if (!profile) return;
  if (!isGameVisibleInCurrentRelease(gameId)) {
    showToast('Bu oyun kalite pilotunda henüz yayınlanmadı.', 'error');
    return;
  }
  const daily = getDailyData(profile);
  const seen = seenQuestionKeysForProfile(state, profile.id);
  ...
  const profileAttempts = attemptsForProfile(state, profile.id);
  const completedSessionCount = profileAttempts.some((attempt) => attempt.gameId === gameId) ? 1 : 0;
  const classTarget = profile.classTarget?.topicIds?.length
    ? profile.classTarget
    : (Array.isArray(profile.teacherFocusTopics) && profile.teacherFocusTopics.length
      ? { topicIds: profile.teacherFocusTopics, focusShare: 0.3 }
      : null);
  let session = createGameSession(gameId, profile, sessionSeed, {
...
function finalizeRound(correct, message, rawScore = correct ? 100 : 25, responseDiagnosis = null) {
  clearTimer();
  const session = ui.session;
  const round = session.rounds[session.currentIndex];
  ...
  const recorded = recordAttempt(state, {
    ...
    selectedOptionIndex: responseDiagnosis?.selectedOptionIndex ?? null,
    selectedOptionText: responseDiagnosis?.selectedOptionText ?? null,
    responseStatus: responseDiagnosis?.responseStatus || null,
    diagnosticStatus: responseDiagnosis?.diagnosticStatus || null,
    misconceptionId: responseDiagnosis?.misconceptionId || null,
    misconception: responseDiagnosis?.misconception || null
  });
  session.answers[session.currentIndex] = {
    ...
    skeletonId: recorded.attempt.skeletonId || null,
    misconceptionId: recorded.attempt.misconceptionId || null,
    diagnosticStatus: recorded.attempt.diagnosticStatus || null
  };
  session.score += score;
...
function evaluateCurrentRound(selectedIndex = null, timedOut = false) {
  ...
  const round = ui.session.rounds[ui.session.currentIndex];
  if (timedOut) {
    finalizeRound(false, `Süre doldu. ${round.explanation}`, 15, diagnoseV11ChoiceResponse(round, null, true));
    return;
  }
  ...
  if (round.kind === 'choice' || round.kind === 'memory') {
    ...
    finalizeRound(correct, correct ? round.explanation : `Doğru yaklaşım: ${round.explanation}`, correct ? 100 : 25, diagnoseV11ChoiceResponse(round, selectedIndex, false));
    return;
  }
