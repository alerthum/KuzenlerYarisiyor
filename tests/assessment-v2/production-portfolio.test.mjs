...
test('ürün portföyü 1-12 hedefini ve mevcut 30 ders motorunu dürüst sayaçlarla gösterir', () => {
  ...
  assert.equal(portfolio.summary.activeEngineCellCount, 30);
  assert.equal(portfolio.summary.canonicalQuestionCount, 1896);
  assert.equal(portfolio.summary.curriculumOutcomeRecordCount, 1871);
  assert.equal(portfolio.summary.coveredOutcomeCount, 1871);
});
...
test('insan incelemesi, oyun uyarlaması ve ürün yayını ayrı kapılar olarak kalır', () => {
  ...
  assert.equal(portfolio.summary.humanReviewQueueCount, 1891);
...
test('ders motorları ortak üreticiye dönüşmeden alan kimliklerini korur', () => {
  ...
  assert.equal(new Set(portfolio.engines.map(row => row.misconceptionCatalog)).size, 30);
