...
import {
  SOCIAL_TIME_TRAVEL_FAMILIES,
  STT_MISREAD
} from '../js/content/families/social-time-travel-families.js';
import {
  FAMILY_TARGET,
...
test('social-time-travel aileleri yapısal olarak geçerlidir', () => {
  for (const family of SOCIAL_TIME_TRAVEL_FAMILIES) {
    const result = validateFamilyDefinition(family);
    assert.ok(result.ok, `${family.familyId}: ${result.errors.join(', ')}`);
  }
});

test('social-time-travel Aşama 04 hedeflerine ulaşır: 12/4/3', () => {
  const report = capacityReport(SOCIAL_TIME_TRAVEL_FAMILIES);
...
test('familyId/skeletonId benzersizdir ve stt-* önekine sahiptir', () => {
  const familyIds = SOCIAL_TIME_TRAVEL_FAMILIES.map((f) => f.familyId);
  assert.equal(new Set(familyIds).size, familyIds.length);
  assert.equal(familyIds.length, 12);
  for (const id of familyIds) assert.match(id, /^stt-/);
  const skeletonIds = SOCIAL_TIME_TRAVEL_FAMILIES.flatMap((f) => f.skeletons.map((s) => s.skeletonId));
    ...
    for (const skeleton of family.skeletons) {
      for (const pathId of skeleton.reasoningPathIds) {
        for (let trial = 0; trial < 3; trial += 1) {
          ...
          assert.ok(round.questionKey?.startsWith('social-time-travel:'), `${skeleton.skeletonId}/${pathId}: questionKey`);
...
test('primary-source forced dogudan zorunluluğu', () => {
  const family = SOCIAL_TIME_TRAVEL_FAMILIES.find((f) => f.familyId === 'stt-primary-source');
  ...
  for (let trial = 0; trial < 10; trial += 1) {
    ...
    assert.equal(round.options[round.answerIndex], 'dogudan');
  }
});

test('misread-taxonomy dört yanlış-okuma türünü kapsar', () => {
  const fam = SOCIAL_TIME_TRAVEL_FAMILIES.find((f) => f.familyId === 'stt-misread-taxonomy');
  ...
  for (const v of Object.values(STT_MISREAD)) {
    assert.ok(seen.has(v), `eksik: ${v}`);
  }
});

test('generateFromFamilies sessionLength:10 üretir', () => {
  const { rounds, audit } = generateFromFamilies(SOCIAL_TIME_TRAVEL_FAMILIES, { seed: 424242, count: 10 });
  assert.equal(rounds.length, 10);
  assert.ok(audit.produced >= 10);
  for (const round of rounds) {
    assert.ok(round.familyId?.startsWith('stt-'));
    assert.equal(round.kind, 'choice');
  }
});

test('CANLI: createGameSession social-time-travel yeni motor (sessionLength 10)', () => {
  const profile = { id: 'stt-live-1', age: 11, grade: 5, skills: {} };
  const session = createGameSession('social-time-travel', profile, 808080, { completedSessionCount: 1 });
  assert.equal(session.rounds.length, 10);
  const familyRounds = session.rounds.filter((r) => r.familyId?.startsWith('stt-') && r.skeletonId);
  assert.ok(familyRounds.length >= 8, `stt-* çoğunluk bekleniyor, alınan: ${session.rounds.map((r) => r.familyId).join(',')}`);
...
test('öğrenci geçmişi aynı iskeleti erken tekrar seçmez', () => {
  const profile = { id: 'stt-cool-1', age: 11, grade: 5, skills: {} };
  const first = createGameSession('social-time-travel', profile, 111, { completedSessionCount: 1 });
  const used = new Set(first.rounds.map((r) => r.skeletonId).filter(Boolean));
  const attempts = first.rounds.map((r) => ({ gameId: 'social-time-travel', skeletonId: r.skeletonId, familyId: r.familyId }));
  const second = createGameSession('social-time-travel', profile, 222, {
...
test('anachronism spot çağdışı ihlali yakalar', () => {
  const fam = SOCIAL_TIME_TRAVEL_FAMILIES.find((f) => f.familyId === 'stt-anachronism');
  const spot = fam.skeletons.find((s) => s.skeletonId.endsWith(':spot-violation'));
  const traps = new Set(['akıllı telefon', 'jet uçak', 'dizüstü bilgisayar', 'metro bileti']);
  for (let i = 0; i < 12; i += 1) {
    const round = spot.generate(seededRandomLike(i * 7 + 2), 'raw-letters');
    const ans = round.options[round.answerIndex];
    assert.ok(traps.has(ans), `spot cevabı çağdışı olmalı: ${ans}`);
  }
});
