...
export function createTargetRound(age, seed) {
  ...
  if (mode === 'group') {
    ...
    return { numbers: [a, b, c, d], target: (a + b) * c - d, solution: `(${a} + ${b}) × ${c} - ${d}` };
  }
  if (mode === 'mixed') {
...
export function createArithmeticRound(age, seed) {
  ...
  return { prompt, answer, options: makeNumericOptions(answer, random) };
}
...
export function createPatternRound(age, seed) {
  ...
  return { sequence, answer, rule, options: makeNumericOptions(answer, random, [-10,-6,-3,-2,2,3,6,10]) };
}
...
export function createGeometryRound(age, seed) {
  ...
  return { prompt, context, answer, explanation, visual, options: makeNumericOptions(answer, random, [-15,-10,-5,-3,3,5,10,15]) };
}
...
export function createProblemRound(age, seed) {
  ...
  if (age <= 10) {
    ...
    if (mode === 'packs') {
      ...
      return { prompt: `${packs} kutunun her birinde ${each} kalem vardır. ${used} kalem kullanılırsa kaç kalem kalır?`, options: makeNumericOptions(answer, random), answer, explanation: `Önce ${packs} × ${each} = ${packs * each}; sonra ${packs * each} - ${used} = ${answer}.` };
    }
    if (mode === 'sharing') {
      const people = randomInt(random, 3, 8), each = randomInt(random, 3, 10), total = people * each;
      return { prompt: `${total} ceviz ${people} çocuğa eşit paylaştırılırsa her çocuk kaç ceviz alır?`, options: makeNumericOptions(each, random), answer: each, explanation: `${total} ÷ ${people} = ${each}.` };
    }
    if (mode === 'time') {
      const start = randomInt(random, 8, 14), duration = randomInt(random, 2, 5), answer = start + duration;
      return { prompt: `Saat ${start}.00’te başlayan bir etkinlik ${duration} saat sürüyor. Etkinlik saat kaçta biter?`, options: makeNumericOptions(answer, random), answer, explanation: `${start} + ${duration} = ${answer}.00.` };
    }
    if (mode === 'money') {
      const count = randomInt(random, 2, 6), price = randomInt(random, 5, 20), paid = Math.ceil(count * price / 10) * 10 + 20, answer = paid - count * price;
      return { prompt: `Tanesi ${price} TL olan defterlerden ${count} tane alan bir öğrenci ${paid} TL veriyor. Kaç TL para üstü alır?`, options: makeNumericOptions(answer, random), answer, explanation: `Toplam ${count} × ${price} = ${count * price} TL; para üstü ${paid} - ${count * price} = ${answer} TL.` };
    }
    ...
    return { prompt: `${total} bilyenin ${numerator}/${denominator}’ü mavidir. Kaç mavi bilye vardır?`, options: makeNumericOptions(answer, random), answer, explanation: `${total} ÷ ${denominator} × ${numerator} = ${answer}.` };
  }
  ...
  if (mode === 'percent') {
    ...
    return { prompt: `${base} öğrencinin %${percent}’i bir kulübe katılmıştır. Kaç öğrenci katılmıştır?`, options: makeNumericOptions(answer, random), answer, explanation: `${base} × ${percent}/100 = ${answer}.` };
  }
  if (mode === 'ratio') {
    const a = randomInt(random, 2, 5), b = randomInt(random, 3, 7), factor = randomInt(random, 4, 12), total = (a + b) * factor, answer = b * factor;
    return { prompt: `Kırmızı ve mavi boncukların oranı ${a}:${b}, toplam boncuk sayısı ${total}’dir. Mavi boncuk sayısı kaçtır?`, options: makeNumericOptions(answer, random), answer, explanation: `Toplam oran ${a + b}; bir pay ${total} ÷ ${a + b} = ${factor}; mavi ${b} × ${factor} = ${answer}.` };
  }
  if (mode === 'speed') {
    const speed = randomInt(random, 40, 90), time = randomInt(random, 2, 5), answer = speed * time;
    return { prompt: `Saatte ${speed} km hızla giden araç ${time} saatte kaç kilometre yol alır?`, options: makeNumericOptions(answer, random, [-50,-30,-20,-10,10,20,30,50]), answer, explanation: `Yol = hız × zaman = ${speed} × ${time} = ${answer} km.` };
  }
  if (mode === 'average') {
    ...
    return { prompt: `Üç sınav notu ${a}, ${b} ve ${adjustedC} olan öğrencinin ortalaması kaçtır?`, options: makeNumericOptions(answer, random), answer, explanation: `(${a} + ${b} + ${adjustedC}) ÷ 3 = ${answer}.` };
  }
  if (mode === 'algebra') {
    const x = randomInt(random, 4, 18), multiplier = randomInt(random, 2, 5), add = randomInt(random, 3, 15), result = multiplier * x + add;
    return { prompt: `${multiplier}x + ${add} = ${result} olduğuna göre x kaçtır?`, options: makeNumericOptions(x, random), answer: x, explanation: `${result} - ${add} = ${result - add}; ${result - add} ÷ ${multiplier} = ${x}.` };
  }
  const price = randomInt(random, 5, 20) * 50, discount = pick([10,20,25,30], random), answer = price * (100 - discount) / 100;
  return { prompt: `${price} TL’lik bir ürüne %${discount} indirim uygulanıyor. İndirimli fiyat kaç TL olur?`, options: makeNumericOptions(answer, random, [-100,-50,-25,25,50,100]), answer, explanation: `İndirim ${price * discount / 100} TL; yeni fiyat ${price} - ${price * discount / 100} = ${answer} TL.` };
}
